...
let rec factorial x acc = if x = 0.0 then acc else factorial (x -. 1.0) (acc *. x)

let rec sin_helper i acc value = 
  if i = 10.0 then acc 
  else let taylor = (((-1.0) ** float_of_int i) * (value **. (2.0 *. i +. 1.0))) /. float_of_int (factorial (2.0 *. i +. 1.0) 1) in 
  sin_helper (i +. 1.0) (acc +. taylor)
